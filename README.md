...
We have simulated the Ising model using the Metropolis method, a Markov Chain Monte Carlo (MCMC) method. 

We implemented the 50x50 2D lattice version of the Ising model and
trained a convolutional neural network (CNN) to perform regession and
classification.  The regression task was of predicting the lattice
temperature while the classification was identifying configurations
